...
function timerPrettyTime(remainingSeconds) {
    ...
    if (remainingSeconds < 0) {
        timerFinished();
		$("#timer_time_remaining").css("display", "inline"); // Show the element when the timer is running
        display = "Finished";
    } else if (remainingSeconds == 0) {
		display = "";
        $("#timer_time_remaining").css("display", "none"); // Hide the element when the timer is 0
    } else {
		$("#timer_time_remaining").css("display", "inline"); // Show the element when the timer is running
...
function timerUpdateTimeRemaining() {
    if (timerFinishedFlag != true) {
        ...
        $("#timer_toggle_button").html('<i class="fas fa-stopwatch mr-1"></i>' + prettyTime);
	}
}
...
function timerButtonsActive() {
	...
	$("#timerActiveGroup").addClass("d-flex");
	$("#timerPausedGroup").removeClass("d-flex");
};
...
function timerLaunch() {
    ...
    req.done(function(response) {
        if (response.result == 'OK') {
            ...
            timerStatus(); // Update the timer status immediately after launching the timer
...
$(document).ready(function(){
    ...
    timerStatus(); // Fetch the timer state on page load
});