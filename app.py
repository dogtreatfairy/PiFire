...
def connect():
    ...
    # Optional but good practice to ensure task starts as soon as someone connects
    with background_task_lock:
        if thread is None:
            print(f"Background task is None. Starting new task (triggered by connect from {sid}).")
            # Start the task and store the returned object (even if we don't check its status later)
            thread = socketio.start_background_task(emit_dash_data)
        # else: # Optional log
            # print("Background task already running or starting.")
...
def disconnect():
    ...
    print(f"Total clients: {clients}")
    # Background task 'emit_dash_data' will stop itself if clients reaches 0
...
def get_dash_data(data={}): # Accept data as a dictionary
    ...
    with background_task_lock:
        # --- MODIFICATION: Simplified check - only relies on thread being None ---
        # If thread is None, it means the background task function has exited (or never started).
        if thread is None:
            print(f"Background task is None. Starting new task (triggered by get_dash_data from {sid}).")
            # Start the task and store the returned object
            thread = socketio.start_background_task(emit_dash_data)
        # else: # Optional log
            # print("Background task is already running or starting.")
...
def emit_dash_data():
    ...
    finally: # --- MODIFICATION: Ensure global 'thread' is reset when task exits ---
...
# ==================================================
#  Handlers for get_app_data and post_app_data
#  (Keep these exactly as they were in the previous version,
#   including any logging or error handling added there)
# ==================================================
...
def get_app_data(action=None, type=None):
    ...
    sid = request.sid
    print(f"Client {sid} requested 'get_app_data' action={action} type={type}")
